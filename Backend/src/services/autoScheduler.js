...
// 👇 NUEVO: importo el servicio que actualiza la hoja Licencias
const { syncLicenciasSheet } = require('./excelLicencias');
...
const RUNNER_PATH = firstExisting([
  path.resolve(ROOT, 'runner.py'),
  path.resolve(PYDIR, 'runner.py'),
]);
const NB_PATH = firstExisting([
  NB_PATH_ENV && path.resolve(ROOT, NB_PATH_ENV),
  path.resolve(PYDIR, 'modelo-12.ipynb'),
]);
const IN_XLSX = firstExisting([
  IN_XLSX_ENV && path.resolve(ROOT, IN_XLSX_ENV),
  path.resolve(PYDIR, 'Datos_v8.xlsx'),
]);
const NB_OUT_DIR = (() => {
  const p =
    firstExisting([
      NB_OUT_DIR_ENV && path.resolve(ROOT, NB_OUT_DIR_ENV),
      path.resolve(PYDIR, 'out'),
      PYDIR,
    ]) || path.resolve(PYDIR, 'out');
  ensureDir(p);
  return p;
...
function isOutputFile(f) {
  return f.startsWith(OUTPUT_PREFIX) && f.toLowerCase().endsWith(OUTPUT_EXT);
}
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      if (code === 0) resolve({ out: outFile, logs: outBuf.trim() });
        ...
        reject(
          new Error(
            `runner.py terminó con código ${code}${
              errBuf ? `\nstderr:\n${errBuf}` : ''
            }${outBuf ? `\nstdout:\n${outBuf}` : ''}`,
          ),
        );
...
function readSheetColumnar(ws) {
  ...
  const items = rows
    .map((r) => {
      ...
      const hora_inicio = String(
        r.hora_inicio || r.inicio || r.HoraInicio || r['hora inicio'] || '',
      ).slice(0, 5);
      const hora_fin = String(
        r.hora_fin || r.fin || r.HoraFin || r['hora fin'] || '',
      ).slice(0, 5);
      return {
        usuario_id: usuario_id ? Number(usuario_id) : null,
        usuario_nombre: usuario_nombre ? String(usuario_nombre).trim() : null,
        fecha,
        hora_inicio,
        hora_fin,
      };
...
function readSheetMatrix(ws, baseMondayYMD) {
  ...
  const isMatrix =
    hdr[0].toLowerCase() === 'trabajador' &&
    hdr
      .slice(1, 8)
      .every((v) => ['1', '2', '3', '4', '5', '6', '7'].includes(String(v)));
  ...
  const items = [];
  const seen = new Map(); // normName -> ordinal por FILA
...
function readWorkbookAllSheets(outPath, baseMondayYMD) {
  ...
  wb.SheetNames.forEach((name, idx) => {
    ...
    if (m) {
      offsetWeeks = Math.max(0, (parseInt(m[1], 10) || 1) - 1);
    } else {
      offsetWeeks = idx; // fallback: orden natural
    }

    const base = baseMondayYMD
      ? toYMD(addDaysLocal(parseYMDLocal(baseMondayYMD), 7 * offsetWeeks))
      : null;
...
async function generateScheduleFromNotebook(fechaInicio, { autoImport = false } = {}) {
  // 👇 NUEVO: antes de correr el notebook, sincronizamos la hoja Licencias
  // usando la fecha de inicio (lunes base) que viene desde el frontend.
  if (fechaInicio) {
    await syncLicenciasSheet(fechaInicio);
  }
